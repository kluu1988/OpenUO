...
internal class SpellweavingSpellbookGump : BaseSpellbookGump
{
    public SpellweavingSpellbookGump(World world, uint item) : base(world, item) { }
    public SpellweavingSpellbookGump(World world) : base(world) { }
    
    protected override void GetSpellToolTip(out int offset)
    {
        offset = 1072042;
    }
    protected override SpellDefinition GetSpellDefinition(int idx) => SpellsSpellweaving.GetSpell(idx);
    ...
    {
        var def = SpellsSpellweaving.GetSpell(offset + 1);
        name = def.Name;
    ...
    protected override void CreateBook()
    {
        ...
        for (int i = 0, spellsDone = 0; i < maxSpellsCount; i++)
        {
            ...
            Label textSpellName = new Label(name, false, 0x0288, font: 6)
            {
                X = topTextX,
                Y = topTextY
            };
