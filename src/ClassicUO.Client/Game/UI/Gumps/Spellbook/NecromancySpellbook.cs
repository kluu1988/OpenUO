...
internal class NecromancySpellbookGump : BaseSpellbookGump
{
    public NecromancySpellbookGump(World world, uint item) : base(world, item) { }
    public NecromancySpellbookGump(World world) : base(world) { }
    protected override void GetSpellToolTip(out int offset)
    {
        offset = 1061390;
    }
    protected override SpellDefinition GetSpellDefinition(int idx) => SpellsNecromancy.GetSpell(idx);

    protected override void GetSpellNames(int offset, out string name, out string abbreviature, out string reagents)
    {
        var def = SpellsNecromancy.GetSpell(offset + 1);
        name = def.Name;
