...
internal class ChivalrySpellbookGump : BaseSpellbookGump
{
    public ChivalrySpellbookGump(World world, uint item) : base(world, item) { }
    public ChivalrySpellbookGump(World world) : base(world) { }
    protected override void GetSpellToolTip(out int offset)
    {
        offset = 1061490;
    }
    protected override SpellDefinition GetSpellDefinition(int idx) => SpellsChivalry.GetSpell(idx);

    protected override void GetSpellNames(int offset, out string name, out string abbreviature, out string reagents)
    {
        var def = SpellsChivalry.GetSpell(offset + 1);
        name = def.Name;
    ...
    protected override void CreateBook()
    {
        ...
        for (int i = 0, spellsDone = 0; i < maxSpellsCount; i++)
        {
            ...
            Label textName = new Label(name, false, 0x0288, font: 6)
            {
                X = topTextX,
                Y = topTextY
            };

            _dataBox.Add(textName, page1);

            if (!string.IsNullOrEmpty(abbreviature))
            {
                textName = new Label(abbreviature, false, 0x0288, 80, 6)
                {
                    X = iconTextX,
                    Y = 34
                };
