...
internal class BushidoSpellbookGump : BaseSpellbookGump
{
    public BushidoSpellbookGump(World world, uint item) : base(world, item) { }
    public BushidoSpellbookGump(World world) : base(world) { }
    ...
    protected override SpellDefinition GetSpellDefinition(int idx) => SpellsBushido.GetSpell(idx);

    protected override void GetSpellNames(int offset, out string name, out string abbreviature, out string reagents)
    {
        var def = SpellsBushido.GetSpell(offset + 1);
        name = def.Name;
