...
internal class NinjitsuSpellbookGump : BaseSpellbookGump
{
    public NinjitsuSpellbookGump(World world, uint item) : base(world, item) { }
    public NinjitsuSpellbookGump(World world) : base(world) { }
    
    protected override void GetSpellToolTip(out int offset)
    {
        offset = 1063279;
    }
    protected override SpellDefinition GetSpellDefinition(int idx) => SpellsNinjitsu.GetSpell(idx);

    protected override void GetSpellNames(int offset, out string name, out string abbreviature, out string reagents)
    {
        var def = SpellsNinjitsu.GetSpell(offset + 1);
        name = def.Name;
